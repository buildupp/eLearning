...
namespace BuildUpp.eLearing.Web.Controllers
{
    ...
    {
        public EnrollmentsController(ILearningRepository repository)
            : base(repository)
        {
        }
        ...
        public IEnumerable<StudentBaseModel> Get(int courseId, int page = 0, int pageSize = 10)
        {
            ...
            query = Repository.GetEnrolledStudentsInCourse(courseId).OrderBy(s => s.LastName);
        ...
        public HttpResponseMessage Post(int courseId, [FromUri]string userName, [FromBody]Enrollment enrollment)
        {
            try
            {
                ...
                if (!Repository.CourseExists(courseId)) return Request.CreateErrorResponse(HttpStatusCode.BadRequest, "Could not find Course");
                ...
                var result = Repository.EnrollStudentInCourse(student.Id, courseId, enrollment);
